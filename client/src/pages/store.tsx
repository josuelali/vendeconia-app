...
export const STORE_PRODUCTS: StoreProduct[] = [
  {
    ...
    benefits: [
      ...
      "Útil en sofás, ropa, camas y asientos",
  ...
  },
  {
    ...
    benefits: [
      "Siete herramientas en formato compacto",
      "Especialmente útil para teclados y auriculares",
  ...
  },
  {
    ...
    benefits: [
      ...
      "Pensado para móvil o tablet",
      "Instalación sencilla en reposacabezas",
      "Envío incluido a España",
    ],
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-6xl px-5 pb-10 pt-14 text-center">
        <span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-cyan-300">
          Selección VendeConIA
        </span>
        <h1 className="mx-auto mt-5 max-w-3xl text-4xl font-black tracking-tight sm:text-5xl">
          Productos útiles. Sin catálogo infinito.
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg text-slate-300">
          Una selección corta de productos prácticos, con precio final claro y envío incluido a España.
        </p>
      </section>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-5 pb-16 md:grid-cols-3">
        {STORE_PRODUCTS.map((product) => (
          <article key={product.slug} className="overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-xl">
            ...
            <div className="p-6">
              <span className="text-xs font-bold uppercase tracking-wider text-cyan-300">{product.badge}</span>
              <h2 className="mt-2 text-xl font-bold">{product.shortName}</h2>
              ...
              >
                Ver producto <ShoppingBag className="h-4 w-4" />
              </Link>
            </div>
          </article>
        ))}
      ...
      <section className="border-t border-white/10 bg-slate-900/50">
        <div className="mx-auto grid max-w-5xl gap-6 px-5 py-10 text-sm text-slate-300 md:grid-cols-3">
          <div className="flex gap-3"><Truck className="h-5 w-5 shrink-0 text-cyan-300" /><div><strong className="block text-white">Envío incluido</strong>Sin costes sorpresa al pagar.</div></div>
          <div className="flex gap-3"><ShieldCheck className="h-5 w-5 shrink-0 text-cyan-300" /><div><strong className="block text-white">Pago seguro</strong>Checkout protegido por Stripe.</div></div>
          <div className="flex gap-3"><PackageCheck className="h-5 w-5 shrink-0 text-cyan-300" /><div><strong className="block text-white">Pedido individual</strong>Cada pedido se gestiona para su destinatario.</div></div>
        </div>
      </section>
    </main>
  );
...
export function StoreProductPage() {
  ...
  if (!product) {
    return (
      <main className="min-h-screen bg-slate-950 px-5 py-20 text-center text-white">
        <h1 className="text-3xl font-bold">Producto no encontrado</h1>
        <Link href="/tienda" className="mt-6 inline-block text-cyan-300 underline">Volver a la tienda</Link>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto max-w-5xl px-5 py-8">
        <Link href="/tienda" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white">
          <ArrowLeft className="h-4 w-4" /> Volver a la tienda
        </Link>

        <div className="mt-8 grid overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl md:grid-cols-2">
          <div className="flex min-h-96 items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900 text-cyan-300">
            <ProductIcon type={product.icon} className="h-32 w-32" />
          </div>
          <div className="p-7 sm:p-10">
            ...
            <ul className="mt-6 space-y-3">
              {product.benefits.map((benefit) => (
                <li key={benefit} className="flex gap-3 text-sm text-slate-200">
                  <Check className="mt-0.5 h-4 w-4 shrink-0 text-cyan-300" /> {benefit}
                </li>
              ))}
            </ul>
            ...
            <button
              ...
              onClick={checkout}
              disabled={loading}
              className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 py-4 text-lg font-black text-slate-950 transition hover:bg-cyan-300 disabled:cursor-wait disabled:opacity-60"
            >
            ...
            <div className="mt-5 flex items-center justify-center gap-2 text-xs text-slate-400">
              <ShieldCheck className="h-4 w-4" /> Pago procesado de forma segura por Stripe
            </div>
          </div>
        </div>
      </div>
...
export function StoreThanks() {
  return (
    <main className="min-h-screen bg-slate-950 px-5 py-24 text-center text-white">
      <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-slate-900 p-10">
        <PackageCheck className="mx-auto h-16 w-16 text-cyan-300" />
        <h1 className="mt-5 text-3xl font-black">Pedido recibido</h1>
        <p className="mt-4 leading-7 text-slate-300">
          El pago se ha completado. Conserva el correo de confirmación de Stripe como justificante del pedido.
        </p>
        <Link href="/tienda" className="mt-7 inline-flex rounded-xl bg-cyan-400 px-5 py-3 font-bold text-slate-950">
          Volver a la tienda
        </Link>
      </div>
    </main>
