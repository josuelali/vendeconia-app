...
const STORE_PRODUCTS: Record<string, StoreProduct> = {
  ...
  "soporte-coche-360": { name: "Soporte 360° para Teléfono o Tablet en Coche", unitAmount: 2499 },
...
export function registerStoreRoutes(app: Express) {
  app.get("/api/store/products", (_req, res) => {
    res.json({ ok: true, products: Object.entries(STORE_PRODUCTS).map(([slug, product]) => ({ slug, name: product.name, price: product.unitAmount / 100, variants: product.variants || [] })) });
  });
  ...
  app.post("/api/store/checkout", async (req: any, res) => {
    try {
      ...
      if (!product) return res.status(400).json({ ok: false, error: "Producto no válido" });
      if (product.variants?.length && !product.variants.includes(variant)) return res.status(400).json({ ok: false, error: "Selecciona una opción válida" });

      const stripe = new Stripe(secretKey);
      const baseUrl = storefrontBaseUrl(req);
      const displayName = variant ? `${product.name} — ${variant}` : product.name;
      const metadata = { storefront: "vendeconia", product_slug: slug, variant: variant || "standard", fulfillment: "manual_dropship_v1" };

      const session = await stripe.checkout.sessions.create({
        mode: "payment",
        line_items: [{ quantity: 1, price_data: { currency: "eur", unit_amount: product.unitAmount, product_data: { name: displayName, description: "Envío incluido a España" } } }],
      ...
      console.error("STOREFRONT_CHECKOUT_ERROR:", { message: err?.message || "unknown", type: err?.type || null });
      return res.status(500).json({ ok: false, error: "No se pudo iniciar el pago. Inténtalo de nuevo." });
    }
  });
}
